...
def test_fix_issue_brief_fallback_includes_replay_evidence() -> None:
    ...
    assert any("jira.update_ticket" in item for item in brief["debug_evidence"])
    assert any("blocked 3" in paragraph for paragraph in brief["description_paragraphs"])
